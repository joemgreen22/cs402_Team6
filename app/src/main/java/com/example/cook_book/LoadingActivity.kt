...
class LoadingActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Thread{
                ...
                if (jArray != null && jArray.length() > 0 && loadingDone) {
                    ...
                    if(totalLoadedItems < jArray.length()){
                        if(totalLoadedItems + 25 > jArray.length()){
                            totalLoadedItems = jArray.length()
                            loadingDone = true
                        } else {
                            totalLoadedItems += 25
                        }
                    }
                    for(i in itemsLoaded until totalLoadedItems){
            ...
            var recipeBundle: Bundle = Bundle()
            recipeBundle.putSerializable("recipeList", recipeList)
            intent.putExtra("myRecipeList", recipeBundle)
